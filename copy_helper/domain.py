...
class Domain:
    ...
    def apply_styles(cls, name, html_copy, priority_block):
        ...
        domain_styles_settings = tools.FileHelper.read_json_data(f'Settings/Domains/{name}/styles.json')
        ...
        html_copy = cls.add_template(name, html_copy, priority_block_text)
        return html_copy
    ...
    def get_priority_block(cls, domain_priority_link_settings, offer_name):
        ...
        sheet_metadata = google_services.GoogleSheets.sheet_service.spreadsheets().get(
            spreadsheetId=priority_products_table_id).execute()
    ...
    def get_and_save_files(cls, domain_name, date, str_copy):
        ...
        tools.FileHelper.write_to_file(
            f'{settings.GeneralSettings.result_directory}/{domain_name}/{date.replace('/', '.')}/{str_copy}.html',
            copy_file_content)

        info_file_text = tools.FileHelper.read_file(
            f'{settings.GeneralSettings.result_directory}/{domain_name}/{date.replace('/', '.')}/info.txt')
